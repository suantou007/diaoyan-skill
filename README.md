...
- **运行时联动调研 Skills**：当任务不是简单功能识别，而是要做判断时，先检索用户 skill 库中的调研相关 skill（例如 `hv-analysis`），再调用合适的 skill 辅助纵向、横向和交汇洞察
- **强制正式报告结构**：HTML 和飞书都必须覆盖 `reference_doc_structure.md` 的公司团队、用户客户、定价、评价、Demo、链接等关键模块
- **支持流程图 / 架构图 / 竞品关系图**：发布飞书时按 `diagram_workflow.md` 优先走 SVG → 飞书画板，交付可编辑图示
- **保留本地输出**：即使飞书暂时不可用，也会留下可复查的截图、`notes.html` 和结构化 manifest
- **默认优先飞书交付**：默认使用 `--as user` 输出飞书文档；飞书不可用时再退回本地 HTML
- **HTML 也要像正式成品**：内置 `notes.html` 设计规范和模板，不把本地交付做成简陋导出页
- **飞书也要像正式成品**：内置飞书版式规范和报告骨架，不把飞书交付做成“截图堆砌 + 文本拼接”
- **飞书组件有明确分工**：内置 callout / table / whiteboard / grid / quote-container 的使用指南
...
- manifest 内应记录 `skills_consulted`（本次检索/使用/跳过了哪些调研相关 skill）
